...
@section('content')
    <x-admin.table filter="true" addbutton="{{route('admin.banks.create')}}" deletebutton="{{route('admin.banks.deleteAll')}}">
            ...
            <x-slot name="tableBody">
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td>{{$row->iban_number}}</td>
                        <td>{{$row->created_at->format('d/m/Y')}}</td>
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.banks.edit' , ['id' => $row->id])}}"><i class="feather icon-edit"></i></a></span>
                            <span class="delete-row text-danger" data-url="{{url('admin/banks/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
