...
@section('content')
    <x-admin.table filter="true"  deletebutton="{{route('admin.reservations.deleteAll')}}">
            ...
            <x-slot name="tableHead">
                ...
                <th>{{awtTrans('هاتف المستخدم')}}</th>
                <th>{{awtTrans('اسم العقار')}}</th>
                <th>{{awtTrans('اسم المقدم')}}</th>
                <th>{{awtTrans('هاتف المقدم')}}</th>
            ...
            <x-slot name="tableBody">
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td>{{$row->estate->name}}</td>
                        <td>{{$row->provider->name}}</td>
                        <td>{{$row->provider->phone}}</td>
                        <td>{{trans('site.'.$row->status)}}</td>
                        <td>{{\Carbon\Carbon::parse($row->created_at)->format('d/m/Y')}}</td>
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.reservations.edit' , ['id' => $row->id])}}">
                                    <i class="feather icon-airplay" title="عرض التفاصيل"></i></a>
                            </span>
                            <span class="delete-row text-danger" data-url="{{url('admin/reservations/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
