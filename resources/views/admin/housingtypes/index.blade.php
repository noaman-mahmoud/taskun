...
@section('content')
    <x-admin.table filter="true" addbutton="{{route('admin.housingtypes.create')}}" deletebutton="{{route('admin.housingtypes.deleteAll')}}">
            <x-slot name="tableHead">
                ...
                <th>#</th>
                <th>{{awtTrans('الاسم بالغه العربية')}}</th>
                <th>{{awtTrans('الاسم بالغه الانجلزية')}}</th>
                <th>{{awtTrans('نوع العقار')}}</th>
            ...
            <x-slot name="tableBody">
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td>{{$row->getTranslations('name')['ar']}}</td>
                        <td>{{$row->getTranslations('name')['en']}}</td>
                        <td>{{$row->estateType->name}}</td>
                        <td>{{$row->created_at->format('d/m/Y')}}</td>
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.housingtypes.edit' , ['id' => $row->id])}}"><i class="feather icon-edit"></i></a></span>
                            <span class="delete-row text-danger" data-url="{{url('admin/housingtypes/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
