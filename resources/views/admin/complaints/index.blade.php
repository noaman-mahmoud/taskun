...
@section('content')
    <x-admin.table filter="true"   deletebutton="{{route('admin.complaints.deleteAll')}}">
            <x-slot name="tableHead">
                ...
                <th>#</th>
                <th>{{awtTrans('عنوان الشكوي')}}</th>
                <th>{{awtTrans('نص الشكوي')}}</th>
                <th>{{awtTrans('هاتف صاحب الشكوي')}}</th>
                <th>{{awtTrans('البريد الالكتروني صاحب الشكوي')}}</th>
            ...
            <x-slot name="tableBody">
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td>{{isset($row->title) ? $row->title : 'الموقع'}}</td>
                        <td>{{$row->complaint}}</td>
                        <td>{{isset($row->user) ? $row->user->phone : $row->phone}}</td>
                        <td>{{isset($row->user) ? $row->user->email : $row->email}}</td>
                        <td>{{$row->created_at->format('d/m/Y')}}</td>
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.complaints.show' , ['id' => $row->id])}}"><i class="feather icon-eye"></i></a></span>
                            <span class="delete-row text-danger" data-url="{{url('admin/complaints/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
