...
@section('content')
    <x-admin.table filter="true" addbutton="{{route('admin.estate-categories.create')}}" deletebutton="{{route('admin.estate-categories.deleteAll')}}">
            <x-slot name="tableHead">
                ...
                <th>#</th>
                <th>{{awtTrans('الاسم بالعربية')}}</th>
                <th>{{awtTrans('الاسم بالانجلزيه')}}</th>
                ...
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.estate-categories.edit' , ['id' => $row->id])}}"><i class="feather icon-edit"></i></a></span>
                            <span class="delete-row text-danger" data-url="{{url('admin/estate-categories/'.$row->id)}}"><i class="feather icon-trash"></i></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
