...
@section('content')
    <x-admin.table filter="true">
            <x-slot name="tableHead">
                ...
                <th>#</th>
                <th>{{awtTrans('الحساب بالغه العربية')}}</th>
                <th>{{awtTrans('الحساب بالغه الانجلزيه')}}</th>
            ...
            <x-slot name="tableBody">
                @foreach($rows as $row)
                    <tr class="delete_row">
                        ...
                        <td>{{$row->getTranslations('account')['ar']}}</td>
                        <td>{{$row->getTranslations('account')['en']}}</td>
                        <td>{{$row->created_at->format('d/m/Y')}}</td>
                        <td class="product-action">
                            <span class="action-edit text-primary"><a href="{{route('admin.accounttypes.edit' , ['id' => $row->id])}}"><i class="feather icon-edit"></i></a></span>
                        </td>
                    </tr>
                @endforeach
            </x-slot>
